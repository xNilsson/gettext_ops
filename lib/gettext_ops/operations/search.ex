defmodule GettextOps.Operations.Search do
  ...
  in .po files by their msgid field, supporting both substring matching
  (case-insensitive by default) and regex patterns.
  ...
  ## Examples
      ...
      {:ok, messages} = GettextOps.Operations.Search.run("welcome", locale: "sv")

      # Regex search
      {:ok, messages} = GettextOps.Operations.Search.run("^Error", locale: "sv", regex: true)

      # Limited results
      {:ok, messages} = GettextOps.Operations.Search.run("button", locale: "sv", limit: 5)
  ...
  @doc """
  Search for translation entries by msgid.
  ...
  ## Examples
      ...
      # Substring search
      GettextOps.Operations.Search.run("Welcome", locale: "sv")
      # => {:ok, [%Expo.Message.Singular{msgid: ["Welcome to Phoenix!"], ...}]}

      # Regex search for entries starting with "Error"
      GettextOps.Operations.Search.run("^Error", locale: "sv", regex: true)
      # => {:ok, [%Expo.Message.Singular{msgid: ["Error: Invalid input"], ...}]}
  ...
  defp do_search(po_path, pattern, use_regex, limit) do
    ...
    case Parser.parse_and_filter(po_path, &Entry.matches_msgid?(&1, search_pattern)) do
      {:ok, messages} ->
